...
my @streams = $coverityWS->get_streams($configProxy, {});

print "id,description\n";

foreach my $stream (@streams) {
  print Dumper($stream) if ($debug);
  my %attributes;
  if ($opt_project) {
     next unless ($stream->{primaryProjectID}->{name} eq $opt_project);
  }

  my @vals = ($stream->{id}->{name},
              $stream->{description},
              );
  print join(',',@vals), "\n";
  $count++;
